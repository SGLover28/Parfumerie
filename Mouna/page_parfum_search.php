...
if(isset($_POST['submit'])){
    ...
    @$search = mysqli_real_escape_string($db,$searchRec);
      if (!empty($search)) 
     { @$sqlSearch= " SELECT * FROM `produit` WHERE NomProduit='$search' AND CategorieProduit='Parfum' ORDER BY NomProduit " ;
  ...
  <nav class="navbar navbar-expand-lg navbar-light colorModif  ">
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <form class="d-flex" action="page_parfum_filtre.php" method="POST">
  ...
  <div class="row">
    ...
    <div class="col-sm-9" >
      <div class="row-card-contenu"> 
        <?php 
                  $existe=0;         
             while ($rowChoix=mysqli_fetch_array($result)) { $existe=1
                      ?>

        
            <div class="column ">
          <div class="content">
          <img  src="<?php echo htmlspecialchars($rowChoix['ImageProduit']); ?>"
                        alt="<?php echo $rowproduit['NomProduit'] ?>" width="100%" />
   
            <h4 class="nom"><?php echo $rowChoix['NomProduit']?></h4>
            <p class="designation"><?php echo $rowChoix['TypeProduit']?> </p>
            <p class="status"><?php echo $rowChoix['Status']?></p>
            
            <P class="prix_contenu"><?php echo $rowChoix['PrixAchat']?> €</P>
             <?php echo '<a  href="produit.php?id='.$rowChoix['NumProduit'].'"> <button class="btn btn_produit">Voir le produit </button></a>'?>
          </div>
        </div>
        <?php   }}
     if ($existe==0) echo "<div style='border:2px solid black;padding:15px;width:900px;' ><h2 >Aucun document correspondant à votre choix n'est trouvé. Veuillez réintroduire votre choix!</h2></div>";

    }?>
    <div style='padding:15px;width:900px;height:800px' ></div>

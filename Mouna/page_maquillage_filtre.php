    ...
    <div class="col-sm-9" >
      ...
      $existe=0;
       if ($longMarque==0 ){
                    ...
                    $produitquery= "SELECT * FROM `produit` WHERE PrixAchat <= $max AND CategorieProduit='Maquillage' ";
                    else if (empty($max)  && !empty($min))
                    $produitquery= "SELECT * FROM `produit` WHERE PrixAchat >= $min AND CategorieProduit='Maquillage'";
                    else {
                        if ($max < $min){
                            echo '<script language="javascript">';
                            echo 'alert("les valeurs min et max sont invalides")';
                            echo '</script>';
                            echo '<meta http-equiv="refresh" content="0;URL=page_maquillage.php">';
                        }else
                    $produitquery= "SELECT * FROM `produit` WHERE PrixAchat >= $min AND PrixAchat<=$max AND CategorieProduit='Maquillage'";}
      ...
      <?php }
        ...
        for ($i= 0;$i<$longMarque;$i++) {
            $marqueRec=mysqli_real_escape_string($marque[$i]);
                    if( empty($max)  && empty($min) )
                    @$produitquery= "SELECT * FROM `produit` WHERE NomProduit='$marque[$i]' AND CategorieProduit='Maquillage' ";
                   else if( !empty($max)  && empty($min) )
                    @$produitquery= "SELECT * FROM `produit` WHERE PrixAchat <= $max AND NomProduit='$marque[$i]'  AND CategorieProduit='Maquillage'";
                    else if (empty($max)  && !empty($min))
                    @$produitquery= "SELECT * FROM `produit` WHERE PrixAchat >= $min AND NomProduit='$marque[$i]'  AND CategorieProduit='Maquillage'";
                    else {
                       if ($max < $min){
                        echo '<script language="javascript">';
                        echo 'alert("les valeurs min et max sont invalides")';
                    ...
                    @$produitquery= "SELECT * FROM `produit` WHERE PrixAchat >= $min AND PrixAchat<=$max AND NomProduit='$marque[$i]'  AND CategorieProduit='Maquillage'";}
                    @$produit=mysqli_query($db, $produitquery);
       ...
       ?>
        <div class="column ">
          <div class="content">
          <img  src="<?php echo $rowproduit['ImageProduit'] ?>"
                        alt="<?php echo $rowproduit['NomProduit'] ?>" width="100%" />
            ...
            <P class="prix_contenu"><?php echo $rowproduit['PrixAchat']?> <i class="fas fa-star"></i></P>
             <?php echo '<a  href="produit.php?id='.$rowproduit['NumProduit'].'"> <button class="btn btn_produit">Voir le produit </button></a>'?>
          </div>
        </div>
        <?php } }}

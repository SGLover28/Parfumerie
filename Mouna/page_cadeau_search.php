...
if(isset($_POST['submit'])){
    ...
    @$search = mysqli_real_escape_string($db,$searchRec);
      if (!empty($search)) 
     { @$sqlSearch= " SELECT * FROM `cadeau` WHERE NomCadeau='$search'  " ;
  ...
  <div class="row">
    ...
    <div class="col-sm-9" >
      ...
      <?php 
            ...
            <div class="column ">
          <div class="content">
          <img  src="<?php echo htmlspecialchars($rowChoix['ImageCadeau']); ?>"
                        alt="<?php echo $rowproduit['NomCadeau'] ?>" width="100%" />
   
            <h4 class="nom"><?php echo $rowChoix['NomCadeau']?></h4>
            <p class="designation"><?php echo $rowChoix['TypeCadeau']?> </p>
            <p class="status"><?php echo $rowChoix['StatusCadeau']?></p>
            
            <P class="prix_contenu"><?php echo $rowChoix['nbPointCadeau']?> <i class="fas fa-star"></i></P>
             <?php echo '<a  href="cadeau.php?id='.$rowChoix['NumCadeau'].'"> <button class="btn btn_produit">Voir le produit </button></a>'?>
          </div>
        </div>
        <?php   }}
    ...
    <div style='padding:15px;width:900px;height:1450px' ></div>

          ...
          <div class="collapse navbar-collapse" id="navbarNav">
            <div id="myHeader" style="background-color: #f1f1f1;">
              ...
              <ul class="navbar-nav linav2">
                ...
                <li class="nav-item">
                  <a class="nav-link  active" href="page_cadeau.php">Cadeaux</a>
                ...
                <li class="nav-item">
                  <a class="nav-link" href="#contact">Accueil</a>
                </li>
        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <?php header('Content-Type: charset=utf-8');
                     $produitquery= "SELECT * FROM `cadeau`  where NumCadeau= $idProduit";
                      $produit=mysqli_query($db, $produitquery);
                    
                      $rowproduit=mysqli_fetch_array($produit);
                      
                       ?>
                <input id="point" name="point" type="hidden"  value=<?php echo $rowproduit['nbPointCadeau']?>>
                  <img  src="<?php echo htmlspecialchars($rowproduit['ImageCadeau']); ?>"
                        alt="<?php echo $rowproduit['NomProduit'] ?>"  style="object-fit: cover;width:640px;height:620px"/>
                </div>
                <div class="col-sm-5" style="margin-left:50px;">
                 <h1 class="nom_produit" ><?php echo $rowproduit['NomCadeau']?> <i style="margin-left: 20px;"class="fas fa-gift"></i></h1>
                 <h3 class="type_produit"><?php echo $rowproduit['TypeCadeau']?></h3>
                 <h4>Stock: <?php echo $rowproduit['StockCadeau']?> </h4>
                 <div class="prixproduit">
                        <ul>
                            <li style="margin-right: 80px;margin-left: 30px;font-size: 21px;">
                                <p><?php echo $rowproduit['VolumeCadeau']?> ml/g</p>
                            </li>
                            <li style="font-size: 21px;">
                                <p><?php echo $rowproduit['nbPointCadeau']?> <i class="fas fa-star"></i></p>
                            </li>
                            ...
                            <?php if( $rowproduit['StockCadeau']==0) { ?>
            ...
            <div class="row">
              ...
              <div class="parag">
                ...
                <div style="height: 80px;padding:20px;  background-color:rgb(153, 33, 51);"> <h2 style="text-align:center;color:#f1f1f1" ><?php echo $rowproduit['NomCadeau']?></h2></div>
...
if (isset($_POST['submit'])){
  ...
  if(!empty($_POST['quantite'])){
      ...
      $stockquery="SELECT * FROM cadeau WHERE NumCadeau=$idProduit";
      $stock=mysqli_query($db,$stockquery);
      $rowStock=mysqli_fetch_array($stock);
      $stockRec=$rowStock['StockCadeau'];
      $newStock=$stockRec-$quantite;
if ($newStock<0){
    echo '<script language="javascript">';
    echo 'alert("le stock est épuisé il reste que '.$stockRec.' exemplaires")';
    echo '</script>';
    echo '<meta http-equiv="refresh" content="0;URL=cadeau.php?id='.$idProduit.'">';
 
}else if ($newStock==0)
$StringStock="épuisé";

$updateStockQuery="UPDATE `cadeau` SET `StockCadeau`='$newStock',`StatusCadeau`='$StringStock' WHERE NumCadeau='$idProduit' ";
$updateStock=mysqli_query($db,$updateStockQuery);

    $prixTotal=$point * $quantite;
    $identifiant=$_SESSION['identifiantClient'];
    $clientquery="SELECT * FROM client WHERE Email='$identifiant'";
    $client=mysqli_query($db,$clientquery);
    $rowclient=mysqli_fetch_array($client);
    ...
    {echo "hehe".$identifiant;
    echo '<script language="javascript">';
    echo 'alert("votre solde de points cadeaux est insuffisant")';
    echo '</script>';
    echo '<meta http-equiv="refresh" content="0;URL=cadeau.php?id='.$idProduit.'">';
    } else{
  ...
  echo "gggg".mysqli_error($db);
  $codeCommandeCadeau=mysqli_insert_id($db);

  $commandeLignequery= "INSERT INTO `cadeaucommande`(`NumCommande`, `NumCadeau`, `QuantiteCommandee`) VALUES ('$codeCommandeCadeau','$idProduit',' $quantite')";
  $commandeLigne=mysqli_query($db, $commandeLignequery);

  $factureQuery="INSERT INTO `facture`( `NumCommande`, `DateFacture`) VALUES ('$codeCommandeCadeau',CURDATE())";
  $facture=mysqli_query($db,$factureQuery);

  $modePaiementQuery="INSERT INTO `reglement`( `NumCommande`, `ModePaiement`, `Montant`, `DatePaiement`, `Observation`) VALUES ('$codeCommandeCadeau','pointsCadeaux',' $prixTotal',CURDATE(),'')";
  $modePaiement=mysqli_query($db,$modePaiementQuery);
    echo '<script language="javascript">';
    echo 'alert("Il vous reste '.$nbpointenTotal.' points cadeaux")';
    echo '</script>';
